...
{
    /// <summary>
    /// Interaction logic for DiseasesWindow.xaml
    /// </summary>
    public partial class DiseasesWindow : Window
    {
            ...
            try
            {
                ...
                using (adapter)
                {
                    ...
                    if (table != null && table.Rows.Count > 0)
                        diseasesDataGrid.ItemsSource = table.DefaultView;
                    else
                        diseasesDataGrid.ItemsSource = null;
                }
            }
            catch (Exception ex)
            {
        ...
        private void SearchButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string query = "SELECT * FROM diseases WHERE name LIKE '%" + tbSearch.Text + "%';";
                SqlCommand command = new SqlCommand(query, Database.sqlConnection);
                SqlDataAdapter adapter = new SqlDataAdapter(command);
        ...
        private bool CheckIfAlreadyExists(string name)
        {
            ...
            connection.Open();
            string tmpQuery = "SELECT * FROM diseases WHERE name=@name";
        ...
        private void AddButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    command.Parameters.AddWithValue("@isVirus", (bool)cbIsVirus.IsChecked ? 1 : 0);
        ...
        private void EditButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (MessageBox.Show("Are you sure you want to update?", "AI Hospital", MessageBoxButton.YesNo, MessageBoxImage.Question, MessageBoxResult.Yes) == MessageBoxResult.Yes)
                {
                    ...
                    CheckIfEmptyInput();
                    if (diseasesDataGrid.SelectedItem != null)
                    {
                        DataRowView data = (DataRowView)diseasesDataGrid.SelectedItem;
                        int id = Convert.ToInt32(data["id"]);

                        string query = "UPDATE diseases " +
                                        "SET name='" + tbName.Text + "', " +
                                        "is_virus=" + ((bool)cbIsVirus.IsChecked ? 1 : 0) + " " +
                                        "WHERE id=" + id + ";";
                        SqlCommand command = new SqlCommand(query, Database.sqlConnection);
            ...
            {
                MessageBox.Show("Error when updating disease data: " + ex.Message, "AI Hospital", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
                Database.CloseConnectionWithDatabase();
        ...
        private void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (MessageBox.Show("Are you sure you want to delete this disease?", "AI Hospital", MessageBoxButton.YesNo, MessageBoxImage.Question, MessageBoxResult.No) == MessageBoxResult.Yes)
                {
                    Database.OpenConnectionWithDatabase();
            ...
            {
                MessageBox.Show("Error when deleting disease data: " + ex.Message, "AI Hospital", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
        ...
        private void diseasesDataGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                if (diseasesDataGrid.SelectedItem != null)
                {
                    ...
                    int id = (int)data["id"];
                    ShowSymptomsOfSelectedDisease(id);

                    bool isVirus = (bool)data["is_virus"];
                    tbName.Text = (string)data["name"];
                    if (isVirus)
                        cbIsVirus.IsChecked = true;
                    else
                        cbIsVirus.IsChecked = false;
                }
            }
        ...
        private void ShowSymptomsOfSelectedDisease(int id)
        {
            ...
            using (adapter)
            {
                ...
                symptomsOfDiseaseDataGrid.ItemsSource = tmpSymptomList;
            }
